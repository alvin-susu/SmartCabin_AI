...
from common.utils import make_context
from main.config import LLM_DEVICE

# 禁用 Hugging Face Tokenizers 库的并行化处理
...
def get_stop_words_ids(chat_format, tokenizer):
    if chat_format == "raw":
        ...
        raise NotImplementedError(f'Unknown chat format: {chat_format}')
...
class ChatLLM(object):
    def __init__(self, model_path):
        ...
        self.generation_config = GenerationConfig.from_pretrained(model_path, pad_token_id = self.tokenizer.pad_token_type_id)
        ...
        self.model = LLM(model = model_path,
                         ...
                         trust_remote_code=True,
                         gpu_memory_utilization=0.6, #根据gpu的利用率自己调整这个比例
                         dtype = "bfloat16")
    ...
    def infer(self, prompts):
        ...
        for q in prompts:
            raw_text, _ = make_context(
                ...
                chat_format=self.generation_config.char_format,
            )
        ...
        for output in outputs:
            ...
            if IMEND in output_str:
                output_str = output[:-len(IMEND)]
            if ENDOFTEXT in output_str:
                output_str = output[:-len(ENDOFTEXT)]
            batch_response.append(output_str)
        torch_gc()
        return batch_response

if __name__ == "__main__":
    qwen7 = "/home/sudi/PycharmProjects/SmartCabin_AI/pre_train_model/Qwen/Qwen-7B-Chat"
    start = time.time()
    llm = ChatLLM(qwen7)
