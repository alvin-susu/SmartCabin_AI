...
3.2.2 BM25召回
BM25算法，通常                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          用于计算两个文本，或者文本与文档之间的相关性。所以可以用于文本相似度计算和文本检索等应用场景。它的主要思想是:对于文本query中的每个词qi，计算qi与候选文本(文档)的相关度，然后对所有词qi得到的相关度进行加权求和，从而得到query与文档document的相关性得分。
...
2. 指标预测：python test_score.py，注意要修改里面的需要预测文件名
注意：vllm只支持linux和mac，建议用linux系统复现本项目！ GPU显存需要40G，2张4090或者一张A100。

3.6 项目改进
1. 采取更多路召回策略，增加TF-IDF召回，bge召回、gte召回和bce-embedding-base_v1召回，然后使用分别使用bge-reranker和bce-reranker-base_v1进行精排；
2. LLM分别采用ChatGLM3-6B, Qwen2-7B-Chat和Baichuan2-7B-Chat作为大模型基座，代码做成可配置。
3. 将抽取后的文档使用LLM重新整理，使得杂乱知识库规整。然后再送入到答案生成模块，这里需要用到prompt提示工程技术。
4. 先用LLM直接生成答案，然后将问题和这个生成的答案拼接，共同完成检索，提升检索效果。
5. 先用LLM先将问题改写和扩充一遍，然后将问题和这个改写后的问题拼接，提升检索效果。
6. 一次给LLM一个检索到的文档，不断优化生成的答案，即利用prompt技术对LLM，context和原答案，优得到优化升级后的答案。

有选择的完成上述项目改进点
这些改进点都可以作为实战调优经验写进简历。
